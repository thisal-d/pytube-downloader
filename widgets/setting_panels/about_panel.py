...
class AboutPanel(ctk.CTkFrame):
    def __init__(self, master: Any = None):
        ...
        self.logo_name_version_frame = ctk.CTkFrame(master=self)
        ...
        self.name_label = ctk.CTkLabel(master=self.logo_name_version_frame, text="")

        self.version_label = ctk.CTkButton(master=self.logo_name_version_frame, hover=False, text="")

        self.developed_by__label = ctk.CTkLabel(master=self, text="Contributors")
        self.dash4_label = ctk.CTkLabel(master=self, text=":")
        ...
        self.explore_title_label = ctk.CTkLabel(master=self, anchor="w", text="")
        ...
        for i in InformationManager.info["contributors"]:
            ...
            if not os.path.exists(profile_images_paths[0]):
                try:
                    ...
                    ImageUtility.download_image(
                        image_url=f"{contributor['profile_url']}.png", output_image_path=profile_images_paths[0]
                    )
            ...
            if not os.path.exists(profile_images_paths[1]) and os.path.exists(profile_images_paths[0]):
                ...
                profile_image_hover = ImageUtility.create_image_with_rounded_corners(
                    ImageUtility.create_image_with_hover_effect(image=profile_image, intensity_increase=40),
                    radius=int(profile_image.width / 2),
                )
            ...
            if os.path.exists(profile_images_paths[1]) and os.path.exists(profile_images_paths[0]):
                ...
                ).grid(row=row, pady=0, padx=(15, 15, 15))
                row += 2
    ...
    def bind_widgets_events(self):
        for explore_btn in self.explore_buttons:
            explore_btn.bind(
                "<Enter>",
                lambda event, btn=explore_btn: btn.configure(
                    fg_color=ThemeManager.get_color_based_on_theme("primary_hover")
                ),
            )
            explore_btn.bind(
                "<Leave>",
                lambda event, btn=explore_btn: btn.configure(fg_color=ThemeManager.get_color_based_on_theme("primary")),
            )
    ...
    def place_widgets(self):
        ...
        self.logo_name_version_frame.grid(
            row=0,
            column=0,
            columnspan=3,
            pady=(50, 0),
